...
public class SuggestionHandler {
	...
	public SuggestionHandler(TyLord bot, Guild guild) {
		this.bot = bot;
		this.guild = guild;
	 ...
	 * @param author the ID of the author who made the suggestion
	 */
	 ...
	 */
	public boolean isSetup() { return suggestions.getChannel() != null; }
	 ...
	 */
	public long getNumber() { return suggestions.getNumber(); }

	/**
	 * Gets the channel ID of the suggestion board.
	 * @return ID of the suggesiton channel.
	 */
	public Long getChannel() { return suggestions.getChannel(); }
	...
	public void respond(SlashCommandInteractionEvent event, int id, OptionMapping reasonOption, SuggestionResponse responseType) {
		...
		try {
			...
			if (suggestions.isResponseDM()) {
				...
				if (author != null) {
					author.openPrivateChannel().queue(privateChannel -> {
						...
						privateChannel.sendMessage(dmText).setEmbeds(editedEmbed).queue();
					});
				}
			}
			...
			// Invalid ID format
			String text = "Could not find a suggestoin with that id number.";
	...
	/**
	 * Suggestion Response Types. Includes the correct color scheme and wording.
	 */
