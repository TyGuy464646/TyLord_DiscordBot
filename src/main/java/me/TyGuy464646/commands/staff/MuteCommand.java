...
/**
 * {@link Command} that adds a muted role to a user in the guild.
 *
 * @author TyGuy464646
 */
public class MuteCommand extends Command {

	public MuteCommand(TyLord bot) {
		super(bot);
		this.name = "mute";
	...
	public void execute(SlashCommandInteractionEvent event) {
		...
		if (!moderationHandler.canTargetMember(target)) {
			event.replyEmbeds(EmbedUtils.createError("This member cannot be muted.")).setEphemeral(true).queue();
			return;
		}
		...
		user.openPrivateChannel().queue(privateChannel -> {
			// Private message user with reason for mute
